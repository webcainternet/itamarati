...
class ControllerModuleTmGoogleMap extends Controller
{
	public function index($setting)
	{
		...
		$this->document->addStyle('catalog/view/theme/' . $this->config->get($this->config->get('config_theme') . '_directory') . '/js/tmgooglemap/tmgooglemap.css');
		$this->document->addScript('//maps.googleapis.com/maps/api/js?' . $key . 'sensor=' . $setting['tm_google_map_sensor'] . '');
		$this->document->addScript('catalog/view/theme/' . $this->config->get($this->config->get('config_theme') . '_directory') . '/js/tmgooglemap/jquery.rd-google-map.js');
		...
		return $this->load->view('module/tm_google_map', $data);
